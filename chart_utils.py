...
class ColoredText:
    undef = {'fontsize': 12, 'color': 'gray', 'line_interval_pix': 6}
    ok = {'fontsize': 12, 'color': 'green', 'line_interval_pix': 6}
    warning = {'fontsize': 12, 'color': 'orange', 'line_interval_pix': 6}
    critical = {'fontsize': 12, 'color': 'red', 'line_interval_pix': 6}
    title = {'fontsize': 12, 'color': 'black', 'line_interval_pix': 6}

    @classmethod
    def draw(cls, lines, result_fname, left_margin=0, **kwargs):
...
def main():
    ...
    lines = [("Поточний стан обладнання:", ct.title),
             ...
             ("Поверх 2: Насос 18.08 - ok", ct.ok)]
    ColoredText.draw(lines, '../tmp/colored_text.png', 20)
