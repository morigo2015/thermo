...
if __name__ == '__main__':
    ...
    def process_file(fname_path):
        ...
        csv_file = open('../tmp/log.csv','a')
        split=os.path.split(fname_path)
        cnt = len(fi.marks_list.qr_marks)
        result = f'{split[0]}, {split[1]}, {cnt}\n'
        csv_file.write(result)
        csv_file.close()
        print(result)
        cv.imwrite(f'../tmp/result/{cnt}_{split[1]}',fi.visual_np)

    total_time_sec = 0
    file_counter = 0
    
    for fname_path in glob.glob('../data/test_qr/flir_*.jpg'):
